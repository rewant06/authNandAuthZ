...
import { 
  Code, Palette, ChevronRight, Smartphone, Database, 
  Globe, Megaphone, Video, ShoppingCart, Brain, Cloud, Lock 
} from "lucide-react";
...
const Services = ({ showTitle = true, className }: ServicesProps) => {
  ...
  return (
    <section ref={sectionRef} className={cn("py-16 relative overflow-hidden", className)}>
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          {servicesData.map((service, index) => (
            <Card
              key={service.id}
              className={cn(
                "overflow-hidden border-2 border-border hover:border-primary/50 transition-all duration-500 cursor-pointer group hover-lift",
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
              )}
            ...
            >
              <CardContent className="p-0">
                <div className="p-6 md:p-8 flex items-center gap-4 md:gap-6 relative overflow-hidden">
                  ...
                  <ChevronRight
                    className={cn(
                      "relative h-6 w-6 md:h-8 md:w-8 text-primary transition-all duration-300",
                      expandedService === service.id ? "rotate-90 scale-110" : "group-hover:translate-x-1"
                    )}
                  />
                </div>
